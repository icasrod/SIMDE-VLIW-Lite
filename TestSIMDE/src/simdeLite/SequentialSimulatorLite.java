...
/**
 * Una versión simple de simulador secuencial basado en SIMDE. Se trata de un port para java que simplemente ejecuta códigos creados en SIMDE.
 * La aplicación necesita un código secuencial en pseudoMIPS de SIMDE (generalmente en un fichero .pla). Opcionalmente requiere una inicialización 
 ...
 * La aplicación asume que el código secuencial está correctamente construido y no contienen errores. 
...
public class SequentialSimulatorLite {
	 ...
	 * @return Un array de enteros con la latencia de cada tipo de unidad funcional, en el orden en que están definidas en {@link FunctionalUnit}
	 * @throws ParameterException Si la lista de latencias no es correcta (número de latencias menor que el número de tipos de unidades funcionales, o alguna latencia menor que 1)
	 */
	...
	private static class Arguments {
		...
		@Parameter(names ={"--latencies", "-l"}, description = "Latencias de cada unidad funcional de la máquina secuencial, expresado como una lista separadas por comas: <#SUMA_ENTERA,#MULT_ENTERA,#SUMA_FP,#MULT_FP,#MEMORIA,#SALTO>.", order = 2)
		...
		@Parameter(names ={"--cachemisspenalty", "-cmp"}, description = "Latencia ADICIONAL cuando se produce un fallo de caché", order = 5)
